...
impl ParsedOutput {
    /// Create a new ParsedOutput with default values
    fn new(raw: &str, metadata: Metadata) -> Self {
        ParsedOutput {
            raw: raw.to_string(),
            structured: json!({}),
            findings: Vec::new(),
    ...
    /// Builder method to set structured data
    fn with_structured(mut self, structured: Value) -> Self {
        self.structured = structured;
        self
    }

    /// Builder method to set findings
    ...
    /// Complete the output with default status if needed
